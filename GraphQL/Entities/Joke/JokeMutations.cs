...
namespace GraphQL.Entities.Joke
{
    ...
    public class JokeMutations
    {
        ...
        // this is private as its a util only used occasionally for migrations
        private async Task<bool> UpdateBadWords([ScopedService] ApplicationDbContext context)
        {
            var badWordsSplit = badWords.Split(", ");
            
            var jokes = from j in context.Jokes select j;
            
            foreach (var joke in jokes.AsEnumerable())
            {
                if (badWordsSplit.Any(joke.Body.Contains))
                    joke.ExplicitContent = true;
            }
